...
const char *soft_ap_ssid = "SCEEMS_Subs_1";
const char *soft_ap_password = "sceems11";
// Client
WiFiClient client;
...
void loop() {
  if (mode == "server") {
    ...
    if(millis() - requestTime > requestTimeDelay) {
      ...
      clientPOST = "account_id=1920384&status=Active";
    ...
    if(kiloWatts != "0.00") {
      ...
      clientPOST = "account_id=1920384&status=Active&account_type_id=1&kilowatts="+kiloWatts;
      // This will send the request to the server
      clientResponse = requestToServer(clientURL, clientPOST);
      Serial.print(clientResponse);
      Serial.println();

/* WiFi Module Code - SUBS2
 * Version 1.0
 * Act as Server when there is no WiFi connection
 * Act as client once new ssid set
 * Can Send notification thru GSM
 * Can receive switch status
 */
...
const char *soft_ap_ssid = "SCEEMS_Subs_2";
const char *soft_ap_password = "sceems22";
// Client
WiFiClient client;
...
//------------------------------
// Meter - Get Kilo Watts
String getkiloWatts() {
  recvWithStartEndMarkers();
  showNewData();
  if(receivedMsg.substring(0,5) == "METER") {
    return "0.000625";
  }
  return "0.00";
}
...
void loop() {
  if (mode == "server") {
    ...
    if(wiFiTimeout == 5) {
      Serial.println("*MODE:backup#");
      serverMode();
    }

    // Client - Request Switch & Notification every 5 seconds
    static unsigned long requestTime = 0;
    if(millis() - requestTime > requestTimeDelay) {
      requestTime = millis();
      // Client - Request 1 (Switch Status)
      ...
      clientURL = "/SCEEMS/api/get_switch_status.php";
      clientPOST = "account_id=2106872&status=Active";
      ...
      // Client - Request 3 (Notification Status)
      // Establishing Connection
      Serial.print  ("Establishing connection to ");
      Serial.println(host);
      // Use WiFiClient class to create TCP connections
      ...
      // We now create a URI for the request
      clientURL = "/SCEEMS/api/check_notifications.php";
      clientPOST = "account_id=2106872";
      // This will send the request to the server
      clientResponse = requestToServer(clientURL, clientPOST);
      Serial.print(clientResponse);
      Serial.println("#");
    ...
    if(kiloWatts != "0.00") {
      ...
      clientPOST = "account_id=2106872&status=Active&account_type_id=3&kilowatts="+kiloWatts;
      // This will send the request to the server
      clientResponse = requestToServer(clientURL, clientPOST);
      Serial.print(clientResponse);
      Serial.println();

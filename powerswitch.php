...
<head>
	...
	<title>S C E E M S - Power Switch</title>
...
<body id="banner">
	...
	<div class="wrappper">
		...
		<section class="container" style =  "padding-top: 150px; padding-bottom:150px">
			...
			<center><span class="ion-power" style ="font-size:120px; color:white;"/></span>
				<h3 style = "color:#fff">POWER SWITCH</h3></center>
				<?php
				...
				$result = $conn ->query($sql);
					while($row = $result->fetch_assoc()){
						...
						if(($tblAccountNumber == $account_id) && ($tblStatus == "ON")){
							echo 			"<div class=round><input type=checkbox id=onoff name=onoff value = 'OFF' />
							<div class=back><label class= but for=onoff><span class = on>I</span><span class = off>0</span></label></div></div>";
						} elseif(($tblAccountNumber == $account_id) && ($tblStatus == "OFF")){
							echo	"<div class=round><input type=checkbox id=onoff name=onoff value = 'ON' checked />
							<div class=back><label class= but for=onoff><span class = on>I</span><span class = off>0</span></label></div></div>";
						}
					}
				...
				<center>
					<button type="submit" class="btn btn-primary btn-round  btn-block" onclick="location.href = 'budget&powersettings.php';">BACK</button></center>
				</section>
			</section>
